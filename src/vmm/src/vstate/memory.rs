    ...
    pub(crate) fn discard_range(
        ...
        match (self.inner.file_offset(), self.inner.flags()) {
            ...
            (Some(file_offset), flags) if flags & libc::MAP_PRIVATE != 0 => {
                ...
                // it's zeroed.
                // Map the replacement MAP_NORESERVE, like the file-backed restore mappings (see
                // `snapshot_file`; only a reserved UFFD-minor hugetlbfs mapping omits it, and the
